...
export function RoomPage({ slug }: { slug: string }) {
	...
	type SearchState = { query: string; posts: SearchResponse["posts"] }
	...
	const [searchState, submitSearch, searchPending] = useActionState(
		async function submitSearch(state: SearchState, fd: FormData) {
			...
			return { query, posts: result.posts }
		},
		{ query: "", posts: [] },
	)
	...
	const [, guessImage] = useActionState(async function guessImage(
		...
		return await guessImageMutation({ roomId: room._id, imageId, name })
	}, null)
		...
		return (
			<NameInputForm
				onSubmit={async (nameValue) => {
					try {
						await join({ roomId: room._id, name: nameValue })
						setName(nameValue)
					} catch (error) {
						console.error(error)
					}
	...
	return (
		<main className="flex min-h-dvh items-start gap-4 bg-gray-950 px-4">
			<section className="sticky top-0 flex w-80 flex-col gap-2 py-4">
				{room.image && (
			...
			<section className="flex flex-1 flex-col gap-2">
				...
				<section className="grid flex-1 grid-cols-[repeat(auto-fill,minmax(150px,1fr))] items-center justify-items-center gap-2 py-4">
						...
						return (
							<form action={guessImage}>
								...
								<button
									key={post.id}
									type="submit"
									className="overflow-clip rounded border border-transparent transition hover:border-pink-500"
								>
									<img src={url} alt="" />
								</button>
								<a
